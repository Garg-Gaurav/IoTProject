...
class MarvelmindHedge (Thread):
    ...
    def handle_binary_data(self, data):
        """Feed binary data into the parser as if received from serial."""
        # Ensure we add individual byte elements, not ints
        for byte in data:
            self._bufferSerialDeque.append(bytes([byte]))
        ...
        if pktHdrOffset == -1:
            print("No packet header found")
            return

        print(f"Header found at offset {pktHdrOffset}")
        try:
            msgLen = strbuf[pktHdrOffset + 4]
            print(f"Message length: {msgLen}")
        except Exception as e:
            print(f"Failed to read message length: {e}")
            return

        # check if enough bytes
        if len(strbuf) < pktHdrOffset + msgLen + 7:
            print("Not enough data for full packet")
            return

        # Now call actual parsing (duplicate or extract from run())
        self.dataEvent.set()
        # (or just print a success marker here)
        print("Data passed basic validation — would be parsed now.")
